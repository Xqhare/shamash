...
pub fn online(config: &Config, logger: &mut Logger) -> Option<ConnectionState> {
    if is_answering_ping(&config.current_target(), config.interval_normal, logger, ConnectionState::Online) {
        thread::sleep(config.interval_normal);
        None
    } else {
        if is_answering_ping(&config.next_target(), config.interval_normal, logger, ConnectionState::Online) {
            thread::sleep(config.interval_normal);
            None
        } else {
            logger.reset();
